...
import { IrsNotificationService } from './irs-notification.service';
...
export class IrsHttpService {
  ...
  getOdata<T>(resourceUrl: string, resourceId?: number) {
    if (resourceId) {
      resourceUrl = `${resourceUrl}/${resourceId}`;
    }
  ...
  putOdata<T>(resourceUrl: string, form: any, id?: number) {
    let response;
    if (form.Id > 0 || id) {
      const formId = id || form.Id;
      response = this.http.put(`${this.odataUrl}${resourceUrl}/${formId}`, form)
        .pipe(catchError((err, caught) => this.handleError(err, caught)));
    } else {
      response = this.http.post<T>(`${this.odataUrl}${resourceUrl}`, form)
        .pipe(catchError((err, caught) => this.handleError(err, caught)));
    }
    return response;
  }

  postOdata<T>(resourceUrl: string, form: any) {
    let response;
    response = this.http.post<T>(`${this.odataUrl}${resourceUrl}`, form)
      .pipe(catchError((err, caught) => this.handleError(err, caught)));
    return response;
  ...
  deleteOdata(resourceUrl: string, id: number) {
    let response;
